...
variation of k-means, which changes the way to calculate the center of each cluster:
 ...
 - **k-medoids** uses the median point of the cluster (so a point of the cluster)
   * pros: is less influenced by noise or outliers.
...
In the package there is, foreach method, a method that iterates over $k$ to find the best fit: \verb|Optimal_Clusters_GMM|, \verb|Optimal_Clusters_KMeans| and \verb|Optimal_Clusters_Medoids|. They look for a $k \in \{1,2,..,max\_clusters\}$, where $max\_clusters$ is a fixed parameter.
...
Note that, not all of the algorithms were executed with all sizes, since some of them are designed only for small or medium size datasets.
...
Note also that the \verb|Clara_Medoids| will be impacted from the choice of sampling's parameters, and we opted for 2 samplings with 5% each.
...
From the chart is clear how a ranking on the execution times of the methods would look like.


From the normal-scaled plot there is also a strong evidence that all the methods, apart the medoids-based excluded, have a linear complexity.
...
At this point, we fitted a linear model for each clustering method in order to find the constants. 
The linear models are all in form $time = a_1 rows + a_2 rows^2 + a_3$, in order to catch both linear and quadratic behaviors.
The auxiliary variable $rows^2$, or \verb|rows_squared|, is precomputed for the entire dataset before the model fitting. 
...
We can see from the t-value that \verb|Cluster_Medoids| execution time goes with the square of the number of rows. 

```{r}
t = df_times %>% filter(method == 'Cluster_Medoids')
model = lm(time ~ rows + rows_squared, data = t)
summary(model)
```


Also \verb|Clara_Medoids| execution time goes with the square of the number of rows, with a much smaller constant than \verb|Cluster_Medoids|. 
...
\verb|KMeans_rcpp|,\verb|GMM|,\verb|MiniBatchKmeans| and \verb|KMeans_arma| execution time, according to the t-values, go linearly with the number of rows.
...
Also \verb|GMM| is linear in the number of rows.
...
Last but not least, we wanted to highlight the different behaviour between centroid-based and medoid-based algorithms.
...
One thing there for sure will emerge is that the centroids will be placed in the middle of each cluster, meanwhile the medoids will be constrained to be one of the elements of the dataset, but probably not really a median point.
...
In the following a 2D PCA on the entire dataset is applied. black class 0 (edible) and red for class 1 (poisonous). 
...
legend("topleft", c("healthy"), fill=c("turquoise1"), border="black")
}
...
We can see that there is no obvious clustering that well separates the labels. There are some denser region with the same color. 
