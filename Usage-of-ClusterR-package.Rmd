...
We can see from the t-value that \verb|Cluster_Medoids| execution time goes with squared complexity. 
...
\verb|KMeans_rcpp|,\verb|GMM|,\verb|MiniBatchKmeans| and \verb|KMeans_arma| execution times, according to the t-values, are linear in the number of input rows.
...
We can see that there is no obvious 2d clustering that well separates the labels. There are some denser region with the same color. 
