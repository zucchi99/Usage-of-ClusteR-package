...
for (r in c(100, 1000, 2000, 4000, 6000, 8000, 100000, 20000, 50000, 75000, 100000, 200000, 300000, 400000, 500000, 800000, 1000000, 2000000)){
  ...
  if (r <= 5000){
    ...
    df_times = measure.time(df_times, "Clara_Medoids", rows=r, f=function() Clara_Medoids(m, 10, samples=4, sample_size=0.1, "euclidean") )
  }
  if (r <= 50000){
...
In order from the slower to the faster (in brackets the approx number of rows clustered in 10 sec):
- Cluster_Medoids (5 * 10^3)
- KMeans_rcpp (5 * 10/4)
- GMM (10^6)
- MiniBatchKMeans (5 * 10^6)
- KMeans_arma (10^7)


```{r}
fig <- plot_ly(data = df_times, x = ~log10(rows), y = ~log10(time), color = ~method)
...
The centroids are placed in the space without constraints.
...
kmeans_rcpp = reorder_data(kmeans_rcpp, "centroids", list("WCSS_per_cluster", "obs_per_cluster"))
kmeans_rcpp$clusters <- NULL #drop it now, it's wrong, use the predict below
m["y_3Mn_rcpp"] = predict(kmeans_rcpp, newdata = m)

{
plot(x=m$X1, y=m$X2, col=m$y_3Mn_rcpp + 1)
points(kmeans_rcpp$centroids, pch="X", cex=1, col="black")
}
```

The medoids are constrained to be on top of items. 
...
kmedoids_pam$silhouette_matrix = change_centroids(kmedoids_pam$silhouette_matrix, kmedoids_pam$medoids[,3], c("clusters", "neighbor_clusters"))
kmedoids_pam$medoids = kmedoids_pam$medoids[,-ncol(kmedoids_pam$medoids)]
m["y_3Md_pam"] = predict(kmedoids_pam, newdata = m)

{
plot(x=m$X1, y=m$X2, col=m$y_3Md_pam + 1)
